-- client_connections
-- connection_broker's supporting table. app/app/services/connection_broker.py
-- (Fernet encrypt/decrypt) has existed since item A, and zacchaeus/stripe_client.py
-- + ops_audit's L3 check both already reference this table -- it was designed
-- earlier this build (as sql/migrations/010_create_client_connections.sql, the
-- pre-CLI-tracked convention) but never actually applied via supabase db push.
-- Applying it for real now, through the CLI-tracked flow this repo uses today.
